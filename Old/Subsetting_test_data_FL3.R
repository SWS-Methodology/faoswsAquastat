# aquastatImputation module

##' This modules intends to take the raw aquastat dataset from SWS  and transform it into a long-format dataset containing:
##' 1 - Variables that cannot be imputed because the full time-series is empity;
##' 2 - Variables that could be imputed by Last Observation Carried Forward
##' 3 - Variables that could be imputed by Linear Interpolation
##'  The output is a ong format dataset
# Data is splited in a MECE way so that all options of either imputation or no imputation can be translated into different datasets
...
# SAVE -----
write.csv(long_all_values, "~./github/Aquastat/modules/faoswsAquastatImputation/output/Datasets/aquastat_test_imputed.csv", row.names = FALSE)
...
data.frame(Rules = paste0("Value_", rule_elements, " := ", "ref$", "Value_", rule_elements))

# All elements
# If elements differ more than 5% of the elements in the reference data (without imputation)
data.frame( UP = paste0("uplim_", "Value_", rule_elements, " < ", " 1.05 * ", "ref$", "Value_", rule_elements))
data.frame(LOW = paste0("lowlim_", "Value_", rule_elements, " < ", " 0.95 * ", "ref$", "Value_", rule_elements))

# Left hand side (calculated variables/indicators)
# If imputed indicators differ more than 5% of the indicators in the reference data (without imputation)
write.csv(data.frame( UP = paste0("uplim_", "Value_", aquastat_indicators, " < ", " 1.05 * ", "ref$", "Value_", aquastat_indicators)),
          "~./github/Aquastat/modules/faoswsAquastatImputation/output/Datasets/crossdata_uplim.csv", row.names = FALSE)

write.csv(data.frame(LOW = paste0("lowlim_", "Value_",aquastat_indicators, " < ", " 0.95 * ", "ref$", "Value_", aquastat_indicators)),
"~./github/Aquastat/modules/faoswsAquastatImputation/output/Datasets/crossdata_lowlim.csv", row.names = FALSE)  
