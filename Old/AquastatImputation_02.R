...
dtt <-
  tbl_df(dt) %>%
  dplyr::group_by(geographicAreaM49, aquastatElement) %>%
...
l4_imputed_values <- lapply(l4_values, function(x){
  if (ncol(x) > 2) {
    fixed <- x[, 1:2]
    imputable <- l4_values[, 3:ncol(l4_values)]
    imputed <- imputeTS::na.locf(imputable)
    cbind(x[, 1:2], imputed)
  } else {
    x[, 1:2]
...
tes <- plyr::mlply(unique(d$geographicAreaM49), .fun = function(x) {
        full_join(l1_values[[x]], l2_imputed_values[[x]]) %>% full_join(l4_imputed_values[[x]]) %>%  full_join(l5_imputed_values[[x]])
 })
names(tes) <- unique(d$geographicAreaM49)
