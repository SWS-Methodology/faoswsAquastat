...
# get the aquastat_update data from the session
...
# data <- fread(paste0(R_SWS_SHARE_PATH, '/AquastatValidation/output/aquastat_update.csv'))
# data[, (colnames(data)) := lapply(.SD, as.character), .SDcols = colnames(data)]
# data[, Value := as.numeric(Value)]
# data[, (colnames(data)) := lapply(.SD, as.character), .SDcols = colnames(data)]
# data[, Value := as.numeric(Value)]
...
# d0 <- merge(d0, cg, by.x = 'geographicAreaM49', by.y = 'm49_code', all.x = TRUE)
...
# All elements regadless indicator
dwider0 <- data.table::dcast(d0, geographicAreaM49 + timePointYears ~ aquastatElement, value.var = 'Value' )
names(dwider0) <- make.names(colnames(dwider0))
...
if (all(region != 'All' & region != 'World')) {

    dw1 <- copy(dwider1)
    countries <- unique(aqua_sdg_agg_4550[sdg_region %in% region, country_el_code])
    sdgcode <- unique(aqua_sdg_agg_4550[sdg_region %in% region, sdg_region])
    colum_to_filter_reg <- unique(aqua_sdg_agg_4550[sdg_region %in% region, column_to_select])
    ...
    names(dww)[3] <- 'regcode'
    transboundary <- unique(aqua_sdg_agg_4550[sdg_region %in% region, transboundary])
    adding <- unique(aqua_sdg_agg_4550[sdg_region %in% region, addition])

    if (all(length(transboundary) > 0 & transboundary == '1')){
    ...
    dt_agg_4550 <- dt_agg_4550[order(regcode,timePointYears)]
...
if (region == 'All') {
      ...
      region_list <- split(aqua_sdg_agg_4550, aqua_sdg_agg_4550$sdg_region)
      ...
      dt2world1 <- dwider0[,.(regionalX4263 = sum(X4263, na.rm = TRUE),
                            regionalX4188 = sum(X4188, na.rm = TRUE),
                            regionalX4549 = sum(X4549, na.rm = TRUE)),
                         by = .(timePointYears)]
      dt2world1$regcode <- '1'
...
if (region == 'World') {
# get for all countries
      dt2world1 <- dworld0[,.(regionalX4263 = sum(X4263, na.rm = TRUE),
                            regionalX4188 = sum(X4188, na.rm = TRUE),
                            regionalX4549 = sum(X4549, na.rm = TRUE)),
                         by = .(timePointYears)]
      dt2world1$regcode <- '1'
...
if (region != 'All' & region != 'World') {
              dw2 <- copy(dwider2)
              sdgcode <- unique(aqua_sdg_agg_4551[sdg_region %in% region, sdg_region])
              colum_to_filter_reg <- unique(aqua_sdg_agg_4551[sdg_region %in% region, column_to_select])
              ...
              cols_to_aggregate <- c(colnames(dww)[grep( '^X', colnames(dww))])
              dww_sum <- dww[regcode %in% sdgcode][, lapply(.SD, sum, na.rm = TRUE), by = c( 'regcode','timePointYears'), .SDcols =   cols_to_aggregate][]
              datatocalc <- dww_sum[complete.cases(dww_sum),]

              calc <- ReadDatatable('calculation_rule')
              proc_rules <- stringr::str_replace_all(calc$calculation_rule, "\\[([0-9]+)\\]", "X\\1")
              sequence <- tail(proc_rules, 6)

              getindicators <- function(){
                d <- copy(datatocalc)
                calc_data <- d
                for(i in sequence) calc_data <- within(calc_data, eval(parse(text = i)))
                calc_data[]
              }

                dt2 <- getindicators()
                dt_agg_4551 <- dt2[complete.cases(dt2), .(regcode, timePointYears, X4551)]
                setnames(dt_agg_4551, 'X4551', 'Value')
                dt_agg_4551 <- dt_agg_4551[order(regcode, timePointYears)]
}

if (region == 'All') {
            # get a list of  data tables with the information necessary for the calculations
            region_list_4551 <- split(aqua_sdg_agg_4551, aqua_sdg_agg_4551$sdg_region)

            # getting aggregations
            sdg_aggregations_list_4551  <- lapply(region_list_4551, function(l){
                        dw2 <- copy(dwider2)
                        # l <- region_list_4551$`419`
                        #countries <- unique(l$country_el_code)
                        sdgcode <- unique(l$sdg_region)
                        colum_to_filter_reg <- unique(l$column_to_select)
                        columns_to_remove <- setdiff(colnames(dwider2)[str_detect(colnames(dwider2), 'code')], colum_to_filter_reg)
      ...
      dww_sum <- dwider0[, lapply(.SD, sum, na.rm = TRUE), by = c('timePointYears'), .SDcols =   cols_to_aggregate][]
      datatocalc <- dww_sum[complete.cases(dww_sum),]
      ...
      dt2 <- getindicators()
      dt2$regcode <- '1'
      dt_agg_4551_world <- dt2[complete.cases(dt2), .(regcode, timePointYears, X4551)]
      setnames(dt_agg_4551_world, 'X4551', 'Value')
