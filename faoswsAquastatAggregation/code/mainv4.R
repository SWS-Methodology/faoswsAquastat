...
if (region != 'All') {
    ...
    if (transboundary == '1'){
              if (sdgcode == "419"){
                dt1 <- dww[regcode %in% sdgcode & geographicAreaM49 %in% countries, .(timePointYears, addition = X4160 + X4162 - X4174)]
              }
              if (sdgcode == "34"){
                # get values that need to be added
                dt1 <- dww[regcode %in% sdgcode & geographicAreaM49 %in% countries, .(timePointYears, addition = X4160)]
              }
              if (sdgcode == "35"){
                dt1 <- dww[regcode %in% sdgcode & geographicAreaM49 %in% countries, .(timePointYears, addition = X4160)]
              }
              if (sdgcode == "150"){
                dt1 <- dww[regcode %in% sdgcode & geographicAreaM49 %in% countries, .(timePointYears, addition = X4168 + X4160)]
              }
              if (sdgcode == "513"){
                dt1 <- dww[regcode %in% sdgcode & geographicAreaM49 %in% countries, .(timePointYears, addition = X4168 + X4160)]
              }
              ...
              if (sdgcode == "145"){
                dtab1 <- dww[regcode %in% sdgcode & geographicAreaM49 == '51', .(timePointYears, addition1 = X4168)]
                dtab2 <- dww[regcode %in% sdgcode & geographicAreaM49 == '31', .(timePointYears, addition2 = X4168)]
                dtab3 <- dww[regcode %in% sdgcode & geographicAreaM49 == '268', .(timePointYears, addition3 = X4160 + X4162)]
              ...
              if (sdgcode == "747"){
                ...
                dtab2 <- dww[regcode %in% sdgcode & geographicAreaM49 == '31', .(timePointYears, addition2 = X4168)]
                dtab3 <- dww[regcode %in% sdgcode & geographicAreaM49 == '268', .(timePointYears, addition3 = X4160 + X4162)]
                dtab4 <- dww[regcode %in% sdgcode & geographicAreaM49 == '729', .(timePointYears, addition4 = X4160 + X4162)]
              ...
              if (sdgcode == "15"){
                dt1 <- dww[regcode %in% sdgcode & geographicAreaM49 %in% countries, .(timePointYears, addition = X4160 + X4162)][!is.na(addition)]
              }
              if (sdgcode == "202"){
                dt1 <- dww[geographicAreaM49 %in% countries, .(timePointYears, addition = X4174)][!is.na(addition)]
              }
              if (sdgcode == "21"){
                dt1 <- dww[geographicAreaM49 %in% countries, .(timePointYears, addition = X4174)][!is.na(addition)]
              }
                ...
                dt_agg <- dt_merge[, .(regcode, timePointYears, Value = regionalX4263/(regionalX4157 + addition - regionalX4549))]
                ...
                dt2 <- dww[regcode %in% sdgcode][,.(regionalX4263 = sum(X4263, na.rm = TRUE),
                                         ...
                                         regionalX4549 = sum(X4549, na.rm = TRUE)),
                                      by = .(regcode, timePointYears)]
                dt_agg <- dt2[, .(regcode, timePointYears, Value = regionalX4263/(regionalX4188 - regionalX4549))]
...
if (region == 'All') {
       ...
       dt_agg <- rbindlist(sdg_aggregations_list)
