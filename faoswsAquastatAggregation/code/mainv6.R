...
##' Date: 08/04/2019
##' Purpose: Aggragates SDG indicators (6.4.1 and 6.4.2) using rules pre-defined
...
if (region != 'All') {
        ...
        dww_sum <- dww[regcode %in% sdgcode][, lapply(.SD, sum, na.rm = TRUE), by = c('timePointYears', 'regcode'), .SDcols =  cols_to_aggregate_sum][]
        dww_mean <- dww[regcode %in% sdgcode][, lapply(.SD, mean, na.rm = TRUE), by = c('timePointYears', 'regcode'), .SDcols =  cols_to_aggregate_mean][]
        dmerge01 <- merge(dww_sum, dww_mean, by = c('timePointYears', 'regcode'))[]

        # calculating 4551 (SDG 6.4.1 Water use efficiency) intermediates
        dt2 <- dmerge01[, .(regcode, timePointYears,
...
if (region == 'All') {
            ...
            sdg_aggregations_list_4551  <- lapply(region_list_4551, function(l){
                        ...
                        dww_mean <- dww2[regcode %in% sdgcode][, lapply(.SD, mean, na.rm = TRUE), by = c('timePointYears', 'regcode'), .SDcols =  cols_to_aggregate_mean][]
