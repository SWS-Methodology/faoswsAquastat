...
dd_rec <- melt(recalc_data_copied, id.vars = id_vars, measure.vars =  measure_vars, variable.name = 'aquastatElement', value.name = 'Value')
setnames(dd_rec, 'Value', 'Value_recalc')
dd_rec[, aquastatElement := substring(aquastatElement, 7)]
dfmerged_recalc <- merge(dd_rec, df_prim_imp, by = c("geographicAreaM49", "timePointYears", 'aquastatElement'), all = TRUE)
