...
data_to_update <- 'aquastat_update'
...
if (data_to_update == 'legacy') {
  ...
  imputKey <- DatasetKey(
    ...
    dimensions = list(
      Dimension(name = "geographicAreaM49",
                keys = GetCodeList('Aquastat', "aquastat_enr", 'geographicAreaM49')[type == 'country', code]),
      Dimension(name = "aquastatElement", keys = GetCodeList('Aquastat', "aquastat_enr", 'aquastatElement')[, code]),
      Dimension(name = "timePointYears", keys = as.character(1961:2018))
    )
  )
  ...
  data_to_update <- readRDS(paste0(R_SWS_SHARE_PATH, '/AquastatValidation/output/update.rds'))
...
dt1 <- unique(rbindlist(lapply(list(data_to_update, data_external, data_quest), ColumnNameHarmonization)))
...
prep_for_calculations <- function(){
            data <- copy_data
...
dataforcalc <- prep_for_calculations()
...
PVapplicableElement <- function(){
        ...
        df_calc[, (zeroelements) := lapply(.SD, function(x) {
          x <- ifelse(all(is.na(x)), 0, x)
        }), .SDcols = zeroelements]
        df_calc[]
}
...
AddIndicators_01 <- function(){
        d <- el2zero_dt
        # dd <- data.table::dcast(d, geographicAreaM49 + timePointYears ~ aquastatElement, value.var = "Value")
        calc_data <- d
...
pv_corrected_data <- pv_correction_01()
