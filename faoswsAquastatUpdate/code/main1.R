...
imputKey <- DatasetKey(
  ...
  dimensions = list(
    ...
    Dimension(name = "aquastatElement", keys = GetCodeList('Aquastat', "aquastat_enr_all_countries", 'aquastatElement')[, code]),
...
data_legacy[, (colnames(data_legacy)) := lapply(.SD, as.character), .SDcols = colnames(data_legacy)]
data_legacy[, Value := as.numeric(Value)]
...
data_quest[, flagAquastat := ifelse(is.na(flagAquastat), "", flagAquastat )]
data_quest[, (colnames(data_quest)) := lapply(.SD, as.character), .SDcols = colnames(data_quest)]
data_quest[, Value := as.numeric(Value)]

# get external data
data_external <- readRDS(paste0(R_SWS_SHARE_PATH, '/AquastatValidation/data/Update/aquastat_external.rds'))
data_external$flagAquastat <- 'E'
data_external <- data_external[, mget(colnames(data_quest))]
data_external[, (colnames(data_external)) := lapply(.SD, as.character), .SDcols = colnames(data_external)]
data_external[, Value := as.numeric(Value)]
...
list_input_data1 <- rbindlist(list(data_legacy[, id := 1L], data_quest[, id := 3L], data_external[, id := 2L]))
all_inputs <- list_input_data1[, .(val_new = Value[id == max(id)], # get the possible duplicated value from the questionnaire
...
copy_data <- copy(all_inputs)
copy_data <- unique(copy_data[, .(geographicAreaM49, aquastatElement, timePointYears, val_new, flagAquastat)])
setnames(copy_data, 'val_new', 'Value')
...
dfmerged_calc <- dfmerged_calc[, .(geographicAreaM49, aquastatElement, timePointYears, Value, flagAquastat)]
dfmerged_calc <- dfmerged_calc[, flagAquastat := ifelse(is.na(flagAquastat), 'C', flagAquastat)]
...
df_imp <- df_imp_pre_copy[, value := as.numeric(ifelse(imp0 == "2", zoo::na.locf(Value),
...
print('AQUASTAT IMPUTATION finished line 299')
...
# FLAG CONVERSION --------------------------------------------------------------------------------------------------------------
print('AQUASTAT FLAG NORMALIZATION started line 374')
...
data_flag_corr <- getFlagObsStatus(dfmerged_final_proc)
...
print('AQUASTAT FLAG NORMALIZATION finished line 421')
...
# SAVE DATA -------------------------------------------------------------------------------------------------------------------
saveRDS(final_data_corr, file.path(Sys.getenv('R_SWS_SHARE_PATH'), 'AquastatValidation/output', 'update.rds'))
stats <- SaveData("Aquastat", "aquastat_update", final_data_corr, waitTimeout = 10000)
