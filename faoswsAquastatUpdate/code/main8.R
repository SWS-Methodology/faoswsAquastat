...
removeDuplicates <- function(data) {
  unique(data) %>%  tbl_df() %>% distinct() %>% setDT()
  }
# LOAD DATA TABLES --------
# get LTA variables
...
write.csv(data_to_save, paste0(R_SWS_SHARE_PATH, '/AquastatValidation/output/Update/aquastat_update.csv'))
...
if (first_time == 'no') {
  ...
  data_quest$id <- 3L
  data_quest <- sameclass(data_quest)
  # country codes in the questionnaire that are not M49 codes
  nonm49 <- setdiff(unique(data_quest$geographicAreaM49), m49_areas)
  #length(fs2m49(as.character(nonm49))) == length(nonm49)
  data_quest[geographicAreaM49 %in% nonm49, geographicAreaM49 := fs2m49(as.character(geographicAreaM49))]
  ...
  ColumnNameHarmonization <- function(d){
    ...
    dd <- dd[order(geographicAreaM49, aquastatElement, timePointYears)]
