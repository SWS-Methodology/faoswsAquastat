...
#test dataset for questionnaire
dq1 <- data_quest[timePointYears == '2015', .(geographicAreaM49, aquastatElement, timePointYears, Value, flagAquastat, id)]
dq2 <- data_quest[timePointYears == '2016', .(geographicAreaM49, aquastatElement, timePointYears, Value, flagAquastat, id)]
dq3 <- data_quest[timePointYears == '2017', .(geographicAreaM49, aquastatElement, timePointYears, Value, flagAquastat, id)]
dq4 <- data_quest[timePointYears == '2017', .(geographicAreaM49, aquastatElement, timePointYears, Value, flagAquastat, id)]
dq1$timePointYears <- '2018'
dq2$timePointYears <- '2019'
dq3$timePointYears <- '2020'
dq4$timePointYears <- '2021'
data_quest <- removeDuplicates(rbindlist(list(dq1, dq2, dq3, dq4)))
...
imputed_data <- sameclass(GetAquaImputations())
...
#
#
# origin <- df_sdg_4550[!maxID %in% c(4L, 5L)]
#
# dfm <- merge(origin[, .(geographicAreaM49, aquastatElement, timePointYears, Value)],
#              copy_data[, .(geographicAreaM49, aquastatElement, timePointYears, flagAquastat, maxID)],
#              by = c("geographicAreaM49", "timePointYears", 'aquastatElement'), all = TRUE)
# dfm[order(geographicAreaM49, aquastatElement, timePointYears)]
#
#
# if (first_time == 'yes'){
# process <- df_sdg_4550[maxID %in% c(4L, 5L)][,
#                                              flagAquastat == 'I']
