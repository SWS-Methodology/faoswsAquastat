...
##' 
##' In the raw data the time-series of country-element combinations are expanded within the first year and 2017
##' 
...
dtt  <-
  ...
  tidyr::complete(timePointYears = as.character(seq(min(as.numeric(timePointYears), na.rm = TRUE), 2017))) %>%
  dplyr::ungroup() %>% 
